    ...
    (set, get) => {
      ...
      return {
        ...
        addImportantDate: (clientId, label, date) => {
          ...
          api.clients.addImportantDate(clientId, label, date).catch(() => {});
        },
        removeImportantDate: (clientId, dateId) => {
          ...
          api.clients.deleteImportantDate(clientId, dateId).catch(() => {});
        },
        ...
        addHearing: (caseId, d) => {
          ...
          api.cases.addHearing(caseId, d).catch(() => {});
        },
        deleteHearing: (id) => {
        ...
        addCaseFile: (caseId, d) => {
          ...
          api.cases.addFile(caseId, d).catch(() => {});
        },
        deleteCaseFile: (id) => {
          set((s) => ({ caseFiles: s.caseFiles.filter((f) => f.id !== id) }));
          toast("success", "تم حذف المرفق");
          api.caseFiles.delete(id).catch(() => {});
        },
        ...
        addNote: (caseId, text) => {
          ...
          api.cases.addNote(caseId, text).catch(() => {});
        },
        ...
        syncWithBackend: async () => {
          try {
            const [clients, cases, agenda, tasks, templates, txs, acts] = await Promise.all([
              api.clients.list(),
              api.cases.list(),
              api.hearings.agenda(),
              api.tasks.list(),
              ...
              api.activities.list(50),
            ]);

            set((s) => ({
              clients: clients.length ? clients : s.clients,
              cases: cases.length ? cases : s.cases,
              hearings: agenda.length ? agenda : s.hearings,
              tasks: tasks.length ? tasks : s.tasks,
              templates: templates.length ? templates : s.templates,
              txs: txs.length ? txs : s.txs,
              activities: acts.length ? acts : s.activities,
            }));
          } catch {
            // Gracefully maintain state if server is not yet online
          }
        },
      };
    },
